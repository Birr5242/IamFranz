(function () {
  ...
  window.addEventListener("keydown", e=>{
    ...
    if(e.code==="KeyK" && !punchAnimating) doPlayerPunch();
  });
  ...
  function doPlayerPunch(){ if(punchAnimating) return; punchAnimating=true; const reach=60; if(Math.abs(player.x-enemy.x)<reach) enemy.hp=Math.max(0,enemy.hp-2.5); setTimeout(()=>{ punchAnimating=false; },PUNCH_COOLDOWN); }

  function update(){
    if(!running||!player||!enemy) return;
    if(!gameStarted) return;

    player.prevX=player.x; enemy.prevX=enemy.x;
    ...
    // KI Bewegung & Intelligenz
    const dx = player.x - enemy.x;
    const dist = Math.abs(dx);
    const difficulty=parseFloat(document.getElementById('difficulty')?.value||"1");
    const jumpChance = 0.01*difficulty, punchRange=60;

    // KI bewegt sich nur wenn nicht zu nah
    if(dist>punchRange){
      enemy.x += dx>0 ? enemy.speed*difficulty : -enemy.speed*difficulty;
      enemy.facing = dx>0?"right":"left";
    } else {
      // KI schlägt nur wenn in Reichweite
      if(!enemy.attackCooldown){
        enemy.attackCooldown=true;
        setTimeout(()=>{ if(Math.abs(enemy.x-player.x)<punchRange) player.hp=Math.max(0,player.hp-2.5); enemy.attackCooldown=false; }, PUNCH_COOLDOWN);
      }
    }

    // KI springt gelegentlich um Angriff auszuweichen
  ...
  function getFrame(c){
    ...
    switch(action){
      case "idle": return faceRight?(images.enemy_stand||images.enemy_stand_back):(images.enemy_stand_back||images.enemy_stand);
  ...
  function draw(){
    ...
    if(images.bg && (images.bg instanceof HTMLImageElement || images.bg instanceof HTMLCanvasElement)) ctx.drawImage(images.bg,0,0,canvas.width,canvas.height);
    ctx.fillStyle="rgba(80,40,140,0.25)"; ctx.fillRect(MIN_X,PLATFORM_Y,MAX_X-MIN_X,6);

    const pf=getFrame(player); if(pf) ctx.drawImage(pf,player.x,player.y,player.w,player.h); else{ ctx.fillStyle="red"; ctx.fillRect(player.x,player.y,player.w,player.h);}
    const ef=getFrame(enemy); if(ef) ctx.drawImage(ef,enemy.x,enemy.y,enemy.w,enemy.h); else{ ctx.fillStyle="blue"; ctx.fillRect(enemy.x,enemy.y,enemy.w,enemy.h);}
